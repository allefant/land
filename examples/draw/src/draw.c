...
static float hueclip(float hue, float pos)
{
    float a = AL_PI / 3.0;
...
static void hue_to_rgb(float hue, float *red, float *green, float *blue)
{
    *red = hueclip(hue, 0);
    *green = hueclip(hue, 2);
    *blue = hueclip(hue, 4);
...
static void draw_star(float middle_x, float middle_y, int n, float r, float start_angle, float spike_length)
{
    float x, y;
    float px, py, pa;
    float a = start_angle;
