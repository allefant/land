...
LandWidgetTheme *classic, *green, *wesnoth
...
def _init(LandRunner *self):
    ...
    wesnoth = land_widget_theme_new("wesnoth_ui/widget.ini")
    ...
    LandWidget* s = land_widget_scrolling_new(desktop, 20, 20, 600, 440)
    LandWidget* v = land_widget_vbox_new(s, 0, 0, 0, 0)
    land_widget_vbox_set_columns(v, 4)
    LandBuffer* b = land_buffer_new()
    land_buffer_cat(b, get_animals())
    land_buffer_strip(b, " \n")
    LandArray* a = land_buffer_split(b, "\n")
    for LandBuffer* i in a:
        char* string = land_buffer_finish(i)
        land_widget_text_new(v, string, False, 0, 0, 10, 10)
        land_widget_text_new(v, "-", False, 0, 0, 10, 10)
        land_widget_text_new(v, "-", False, 0, 0, 10, 10)
        land_widget_text_new(v, "-", False, 0, 0, 10, 10)
        land_free(string)
    land_array_destroy(a)
    land_buffer_destroy(b)
    land_widget_scrolling_autohide(s, 1, 0, 2)
...
def _tick(LandRunner *self):
    ...
    if land_key_pressed(' '):
        if theme == green: theme = wesnoth
        elif theme == wesnoth: theme = classic
        else: theme = green
...
def get_animals -> str: return _animals
