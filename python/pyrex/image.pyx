...
#    LandImage *land_find_image(char *name)
    void land_image_draw_scaled_rotated_tinted(LandImage *self, float x, float y, float sx, float sy, float a, float r, float g, float b, float a)
#    void land_image_draw_scaled_rotated(LandImage *self, float x, float y, float sx, float sy, float a)
...
#    void land_image_draw_scaled_tinted(LandImage *self, float x, float y, float sx, float sy, float r, float g, float b)
...
#    void land_image_init()
    ...
    void land_image_grab_into(LandImage *self, int x, int y, int tx, int ty, int
            w, int h)

def load_all(pattern):
    pass
    #return land_load_images(pattern)
    #TODO: Wrap all of them and put into a python dictionary, probably not using
    #the C function at all
...
cdef class Image:
    def __init__(self, *args):
        if len(args) == 0:
            ...
            other = args[0]
            self.wrapped = land_image_new_from((<Image>args[0]).wrapped, args[1], args[2],
                    args[3], args[4])
        else:
    ...
    def colorize(self, mask): land_image_colorize(self.wrapped, (<Image>mask).wrapped)
    ...
    def replace_with(self, other):
        self.wrapped = (<Image>other).wrapped
